...
public  class UserService implements  IUser<User> {
    ...
    public UserService() {
        this.conn = MyDb.getInstance().getCon();
    }
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
                    ...
                    return switch (role) {
                        ...
                        case "localCom" ->
                                new LocalCom(id, email, password, rs.getString("Fname"), rs.getString("Lname"), rs.getInt("phone"),
        ...
        try (PreparedStatement statement = conn.prepareStatement(query)) {
            ...
            try (ResultSet res = statement.executeQuery()) {
                if (res.next()) {
                    ...
                    boolean status = res.getBoolean(12);
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    ...
                    final User utilisateur = switch (role) {
                        case "admin" -> new Admin(id, email, password);
                        case "tourist" ->
                                new Tourist(id, rs.getString("Fname"), rs.getString("Lname"), email, password, rs.getInt("phone"), role, rs.getString("Bio"), rs.getString("Preferences"));
                        case "localCom" ->
                                new Tourist(id, rs.getString("Fname"), rs.getString("Lname"), email, password, rs.getInt("phone"),
    ...
    public void deactivateOrActivateUser(int userId) throws SQLException {
        User user = searchByUid(userId);
        ...
        if (user != null) {
            ...
            // Optionally, you can update the User object's status as well
            user.setStatus(newStatus);
        }
    }

    public void deactivateUser(int userId) throws SQLException {
        deactivateOrActivateUser(userId);
    }
...
// ...
    ...
    public void updateUserStatus(int userId, boolean status) throws SQLException {
        ...
        try (PreparedStatement stmt = conn.prepareStatement(query)) {
            // Use 1 for true and 0 for false in a TINYINT column
